/**
 * Ecocode plugin for OpenCode.ai
 *
 * Injects ecocode bootstrap context via system prompt transform.
...
const ecocodeSkillsDir = path.resolve(__dirname, "../../skills");

// Module-level cache — SKILL.md does not change during a session
let _bootstrapCache = undefined;

export const EcocodePlugin = async ({ client, directory }) => {
...
${toolMapping}
  ...
  return {
    config: async (config) => {
      ...
      if (!config.skills.paths.includes(ecocodeSkillsDir)) {
        config.skills.paths.push(ecocodeSkillsDir);
      }
    },
