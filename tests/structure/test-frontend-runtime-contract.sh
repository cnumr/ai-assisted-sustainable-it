...
contains_regex "schéma d'entrée documente les types" "$SKILL" '\| *Objet *\| *Clés autorisées *\| *Clés requises *\| *Types exacts *\|'
...
contains_regex "schéma page définit sept arrays" "$SKILL" '\| *page *\| `nom`, `url`, `metriques`, `ecoindex`, `ecarts_greenit`, `performance`, `developpement_web`, `a_verifier`, `couverture`, `deduplication`, `capture`, `limites` *\| deux strings ; deux objects ; sept arrays ; string ou null *\|'
